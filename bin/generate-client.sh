...
generate_client() {
  ...
  docker run --rm -v $PWD:/local -w /local -u "$(id -u $USER)" \
    ...
    --additional-properties nullSafeAdditionalProps=true,supportsES6=true,typescriptThreePlus=true,withInterfaces,withNodeImports,npmName=$target_npm_name,modelPropertyNaming=original,enumPropertyNaming=original \
