...
describe('Schema wise permissions', () => {
  ...
  it('An admin can get and update all services', () => {
    ...
    expect(authz.validateWithCasl('create', 'Service', 'mercury', data)).to.be.false
    expect(authz.validateWithCasl('delete', 'Service', 'mercury', data)).to.be.false
    expect(authz.validateWithCasl('read', 'Service', 'mercury', data)).to.be.true
    expect(authz.validateWithCasl('update', 'Service', 'mercury', data)).to.be.true
  })
  ...
  it('A team can only get its own service', () => {
    ...
    expect(authz.validateWithCasl('delete', 'Service', 'mercury', data)).to.be.false
    expect(authz.validateWithCasl('read', 'Service', 'mercury', data)).to.be.true
    expect(authz.validateWithCasl('update', 'Service', 'mercury', data)).to.be.false
  })
})
...
describe('Ownership wise resource permissions', () => {
  ...
  it('A team cannot update service from another team', () => {
    ...
    const authz = new Authz(spec).init(sessionTeam)
    expect(authz.validateWithCasl('update', 'Service', 'venus', data)).to.be.false
  })
